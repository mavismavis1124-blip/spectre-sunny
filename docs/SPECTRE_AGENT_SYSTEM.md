...
- Consider day mode when adding or changing UI/styling: ensure day-mode overrides (e.g. `.app.app-day-mode` or `*.day-mode.css`) exist or are updated so the app looks correct in light theme too
...
7. **DAY MODE IS MANDATORY** - See Day Mode System below

---

### 🌗 DAY MODE SYSTEM (CRITICAL)

**Every UI change MUST include day mode styles.** Dark text on dark background = broken. Light text on light background = broken.

**Day Mode Color Palette**:
```css
/* Day Mode Backgrounds */
--day-bg-primary: #f5f5f7;        /* Light gray - page background */
--day-bg-secondary: #ffffff;      /* White - card backgrounds */
--day-bg-tertiary: #f0f0f5;       /* Slightly darker - elevated surfaces */
--day-bg-glass: rgba(0,0,0,0.02); /* Light glassmorphism */

/* Day Mode Text (DARK text for readability) */
--day-text-primary: #0f172a;      /* Near black - headings, important */
--day-text-secondary: #334155;    /* Dark gray - body text */
--day-text-tertiary: #64748b;     /* Medium gray - muted/labels */

/* Day Mode Borders */
--day-border-subtle: rgba(0,0,0,0.06);
--day-border-glass: rgba(0,0,0,0.1);
--day-border-strong: #e2e8f0;
```

**Selector Hierarchy** (use appropriate level):
```css
/* Component-level (most specific) */
.my-component.day-mode { }

/* Page-level */
.welcome-page.day-mode .my-element { }

/* App-level (global fallback) */
.app.app-day-mode .my-element { }
.app.welcome-day-mode .my-element { }
```
...
**⚠️ MANDATORY DAY MODE CHECKLIST** (run for EVERY CSS change):
```
□ Text color: Dark enough for light backgrounds? (#0f172a, #334155)
□ Background: Light/white instead of dark? (#f5f5f7, #ffffff)
□ Borders: Using dark alpha instead of white alpha? (rgba(0,0,0,0.08))
□ Icons/SVGs: Contrast inverted or colored for visibility?
□ Shadows: Lighter, softer shadows? (rgba(0,0,0,0.05))
□ Glass effects: Adjusted opacity for light theme?
□ Hover states: Still visible on light background?
□ Focus rings: Visible against light background?
□ Placeholder text: Dark enough to read?
□ Scrollbars: Styled for light theme?
```

**JSX Pattern** (components receive dayMode prop):
```jsx
<div className={`my-component${dayMode ? ' day-mode' : ''}`}>
```

**CSS Pattern** (always pair dark + day):
```css
/* Dark mode (default) */
.my-component {
  background: #12121a;
...
**Common Day Mode Fixes**:
```css
/* White text on light bg? Fix: */
.app.app-day-mode .element { color: #0f172a; }

/* Invisible borders? Fix: */
.app.app-day-mode .card { border-color: rgba(0,0,0,0.1); }

/* Washed out icons? Fix: */
.app.app-day-mode .icon { filter: invert(0.8); }
/* OR use explicit dark color */
.app.app-day-mode .icon { color: #334155; }
```
...
Day Mode: [✓ dark mode styles | ✓ day mode overrides]

[CSS implementation - BOTH dark and day mode]
```
